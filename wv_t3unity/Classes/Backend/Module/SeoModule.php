<?php
namespace WebVision\WvT3unity\Backend\Module;
...
class SeoModule extends \TYPO3\CMS\Backend\Module\AbstractFunctionModule {
    ...
    public function main() {
        ...
        if ($id) {
            ...
            $this->pObj->doc->getPageRenderer()->addJsFile(\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extRelPath('wv_t3unity') . 'Resources/Public/JavaScript/SeoModule.js');
            ...
            //$tree->addField('unity_path', 1);
            $tree->addField('path', 1);
    ...
    protected function renderSEOTable($tree) {
        ...
        return '<table border="0" cellspacing="1" cellpadding="0" id="tx-wv_t3unity" class="lrPadding c-list typo3-dblist">' . $output . '</table>';
    }
    ...
    protected function renderRowContent($item, $rowTitle = NULL) {
        ...
        if ($cmd != 'edit') {
            ...
            $row1[] = '<td>Keywords:</td><td><input name="' . $fName . '[keywords]" value="' . htmlspecialchars($item['keywords']) . '" type="text" size="67" maxlength="180" autocomplete="off" class="seoKeywords"/></td>';
    ...
    protected function loadLanguageOverlays($uidList) {
        ...
        $res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
            'uid, pid, sys_language_uid, title, keywords, description',
    ...
    protected function saveChanges() {
        ...
        foreach($seoData as $tbl => $res) {
            ...
            foreach ($res as $uid => $item) {
                $uid = intval($uid);
                if (empty($item['keywords']) && empty($item['description'])) {
                    $emptyItems[] = $uid;
                    continue;
                }
